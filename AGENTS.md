...
- **License:** GPL-3.0.
- **Current version:** `1.2.5+56` in `pubspec.yaml`, `1.2.5.0` in `msix_config.msix_version`, and `1.2.5` in `installer.iss`.
- **Latest tag at the time this guide was written:** `v1.2.5`.
...
- `test/storage_hardening_test.dart` (missing-file null loads, corrupt-file typed exceptions with byte-unchanged files, atomic saves without `.tmp-*` leftovers, concurrent-save regression for intimacy/weight/todo/exchange rates)
- `test/add_record_dialog_test.dart` (editing a record whose partner was deleted builds and preserves the untouched partner id)
...
- Every module data-file storage (`FinanceStorage`, `IntimacyStorage`, `WeightStorage`, `TodoStorage`'s `todo_data.json` path, and `ExchangeRateStorage`) serializes saves through a static write queue and writes atomically via validated tmp-then-rename (`DataFileSafety.writeValidatedDataJson`; finance keeps its own equivalent `_atomicWriteJson`). Overlapping un-awaited saves (e.g. partner deletion firing several home-page callbacks) can no longer interleave truncate-writes and garble JSON.
- `load()` returns `null` only when the data file does not exist; an existing but unreadable file throws a typed `FinanceStorageException` / `IntimacyStorageException` / `WeightStorageException` / `TodoStorageException` so corrupted data is never silently treated as an empty dataset. Each home page mirrors `finance_page.dart`: it shows a blocking load-error view, refuses `_saveData` with a `<module>DataWriteBlocked` SnackBar while unreadable, and recovers automatically once the file is readable again (the `AutoSyncService` reload listener re-runs `_loadData`). Read-only Todo/Weight reminder callers catch the exception and skip that pass.
...
- Todo, Finance, and Weight handlers return HTTP 500 `{"error":"data_unreadable"}` when an existing data file cannot be parsed; missing files still use the endpoint's documented empty-data behavior, and write endpoints abort before saving.
- Endpoints:
  - `GET /ping`
