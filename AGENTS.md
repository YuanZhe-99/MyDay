...
- The manage menu has a fourth **Body** entry opening `views/body_page.dart` (`BodySettingsPage`), which hosts the shared `widgets/body_section.dart` (`BodySectionView`) in user mode. Partner detail pages (`_FilteredRecordsPage` with a `partnerId`) render **Records | Body** tabs; the Records tab is the pre-existing summary/trend/list content, the Body tab is the same shared widget in partner mode. Toy detail pages keep the single-scroll layout and never get a body tab. Partner creation/editing dialogs are unchanged and carry `body` through reconstruction (edit dialog, break-up).
- User bust/waist/hip mirror the latest Weight record (datetime-descending). Editing them first shows a warning that changes sync to the Weight module and create a new weight record, with a "Do not remind me again" checkbox; the opt-out is the local-only `storage_config.json` key `intimacyBodyWeightSyncWarningDisabled`, mirrored by a switch at the bottom of the Body page (unchecking re-arms the warning). A confirmed editing burst debounces into exactly one new `WeightRecord` reusing the latest weight (0 when none exists) and the displayed bust/waist/hip; historical weight records are never modified. Partner measurements live on `Partner.body` and never touch the Weight module.
