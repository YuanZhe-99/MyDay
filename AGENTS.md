...
Operating guide for agents working on **MyDay!!!!!**. This file holds **only** rules about how to
work here. Everything describing what the code *is* or *does* lives in `doc/en-us/` — see
[Where to read what](#where-to-read-what).

MyDay!!!!! is a privacy-first daily-life companion (Flutter; Windows, Android, iOS, macOS) covering
Todo, Finance, Intimacy, and Weight. Treat the user's message as the change request: plan, implement,
verify, report.
...
- **UTC timestamps** for anything compared across devices (`modifiedAt`, settings timestamps).
  Local-time values break sync conflict detection.
- **Preserve unknown JSON fields.** MyDay's merge output is *not* self-preserving: unknown fields are
  re-applied at write time from the schemas in `shared/utils/json_preservation.dart`. Adding a field
  to a model means adding it to the matching schema, or it will be treated as unknown.
- **File I/O goes through the storage hubs** (`TodoStorage.getAppDir()` and friends) so custom
  storage paths keep working.
- **Data writes are serialized and atomic.** Corrupt or unreadable data files must raise typed
  exceptions rather than being treated as empty — silently treating a corrupt file as empty has
  caused data loss here before (see `v1.1.0` and `v1.2.5`).
...
- **The shared-service facades keep their public shape.** `WebDAVService`, `BackupService`,
  `ImportExportService`, `AutoSyncService`, and `DataFileSafety` are thin wrappers over
  `myapps_data`. If a change seems to require editing a facade's public API, stop — the facade exists
  so call sites and tests keep working, and behavior changes belong in the package.
...
- `sync_progress.dart`, `sync_wake_lock.dart`, the generic half of `sync_merge.dart`, and the engine
  half of `utils/json_preservation.dart` are re-export shims. Do not reintroduce implementations.

MyDay-specific behavior that deliberately stayed app-side, and must not be "unified" away:

- The **finance forced-balance migration** runs as the module's `postMergeTransform` — after the
  merge *and* after conflict resolution, on both the sync and finalize paths.
- **Exchange rates** use a whole-file union merge that can never produce a record conflict.
- **The daily backup is driven by `ReminderService`'s 30-second loop**, which is why
  `AutoSyncService` passes `onPeriodicTick: null`. Wiring the backup into the scheduler would
  double-drive it.
...
**This repo's branch is `main`** — MyAnime and MyDevice use `master`, so a shared release script that
hardcodes a branch name will silently fail here while still pushing the tag. Push `HEAD` or check
...
**Never commit:** secrets, credentials, WebDAV configuration, signing keys, personal data of any
module (Todo, Finance, Intimacy, Weight), generated app data, or local-only machine addresses.
