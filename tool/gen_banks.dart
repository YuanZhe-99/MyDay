...
void main() async {
  ...
  if (!dir.existsSync()) {
    stdout.writeln('banks-db not found at $banksDbDir');
    exit(1);
  }
  ...
  for (final countryDir in dir.listSync().whereType<Directory>()) {
    for (final file in countryDir.listSync().whereType<File>()) {
      ...
      entries.add({
        ...
        'localTitle':
            (raw['localTitle'] as String?) ?? (raw['engTitle'] as String),
  ...
  final extras = <Map<String, String>>[
    ...
    },
    {
      ...
      'country': 'jp',
      'localTitle': '三井住友銀行',
      'engTitle': 'Sumitomo Mitsui Banking',
      'color': '#4caf50',
      'domain': 'smbc.co.jp',
    },
    {
    ...
    },
    {
      ...
      'country': 'jp',
      'localTitle': 'SBI新生銀行',
      'engTitle': 'SBI Shinsei Bank',
      'color': '#0066cc',
      'domain': 'sbishinseibank.co.jp',
    },
    {
      'id': 'aeon-bank',
      'country': 'jp',
  ...
  stdout.writeln('Written ${entries.length} banks to ${outFile.path}');
}
