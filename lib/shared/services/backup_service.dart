/// Purpose: MyDay's local backup API, now a facade over the shared
...
/// still driven by `ReminderService`'s 30-second loop, not by the auto-sync
/// scheduler.
library;
...
class BackupService {
  ...
  static final Map<String, String> modules = {
    for (final module in todoModuleRegistry.modules)
  ...
  static final shared.BackupEngine _engine = shared.BackupEngine(
    storage: TodoStorageAdapter(appDir: _getAppDir),
