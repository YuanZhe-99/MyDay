/// Purpose: MyDay's WebDAV sync API, now a facade over the shared
...
/// Notes: PLAN.md P3.2.3. Every public member kept its name, signature, and
/// semantics so call sites, the conflict dialogs, and the existing tests
/// compile and behave unchanged (I7). The five data modules — including the
/// finance forced-balance migration, the whole-file exchange-rate merge, and
/// the schema-driven unknown-field preservation — are described in
...
class SyncResult {
  ...
  /// Purpose: Return whether conflicts need manual resolution.
  /// Inputs: None.
  /// Returns: `bool`.
  /// Side effects: None.
  /// Notes: None.
...
class PendingSync {
  /// Pending todo merge, when daily or once tasks conflicted.
  final TodoMergeResult? todoMerge;

  /// Pending finance merge, when accounts, categories, transactions, or
  /// subscriptions conflicted.
  final FinanceMergeResult? financeMerge;

  /// Pending intimacy merge, when any of its five containers conflicted.
  final IntimacyMergeResult? intimacyMerge;

  /// Pending weight merge, when weight records conflicted.
...
class WebDAVService {
  ...
  static final shared.WebDavSyncEngine _engine = shared.WebDavSyncEngine(
    ...
    defaultRemotePath: todoDefaultRemotePath,
  );
  ...
  /// Notes: A missing `remotePath` still defaults to `/MyDay`.
  ...
  /// Inputs: `config`, `pending`, `resolutions` (record ID to chosen record).
  ...
  /// The finance forced-balance migration still runs after resolution.
  ...
  /// Exchange rates merge whole-file and never conflict, so they never appear.
  static SyncResult _toSyncResult(shared.EngineSyncResult result) {
    ...
    return SyncResult(
      ...
      pending: pending == null
          ? null
          : PendingSync(
              todoMerge: pending.forModuleId('todo')?.state as TodoMergeResult?,
              financeMerge:
                  pending.forModuleId('finance')?.state as FinanceMergeResult?,
              intimacyMerge:
                  pending.forModuleId('intimacy')?.state
                      as IntimacyMergeResult?,
              weightMerge:
                  pending.forModuleId('weight')?.state as WeightMergeResult?,
