...
`ImageService` 选择本地图像、下载 logo/照片、复制内置银行标志（完整版构建），以 UUID 文件名存储在 `images/` 下、解析相对路径，并拒绝微型占位下载（防御损坏/空图像 URL 被保存为真实图像）。复制来的内置标志是普通的 `images/` 文件，可能是 `.svg`；备份 blob 保留每个图像的扩展名（`<sha256><ext>`），ZIP 导出/导入按文件名复制 `images/` 文件，因此 SVG 的备份和恢复无需任何格式变化。
