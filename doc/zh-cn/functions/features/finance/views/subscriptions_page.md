...
本文件是 [财务](../../../../features/finance.md#views-and-analysis-page) 描述的取消/恢复状态机的具体实现：订阅可以立即或到期时取消；待定的到期时取消可以在原地撤销；完全历史（非激活）订阅通过把其设置复制进**新**激活订阅来恢复，而不是修改旧的。实现那个状态机的大多数方法（`_restoreSubscription`、`_undoAtExpiryCancellation`、`_copyRestoreSubscription`、`_doCancelSubscription`、`_insertNewSubscription`、`_editSubscription`）分类为 Tier A，连同重排逻辑、历史交易导入器和 `_SubscriptionTile` 的状态标签计算（它直接反映订阅当前取消状态）。自 v1.4.3 起，三项摘要统计、排序和即将续费过滤器位于 [`subscription_summary.dart`](../services/subscription_summary.md)，图块的头像位于 [`subscription_avatar.dart`](../widgets/subscription_avatar.md)，因为财务主页的双栏订阅概览显示同样的数字和同样的头像。
...
| [`_active`](#_active) | getter（`_SubscriptionsPageState`） | A | 经 `sortSubscriptions` 按当前排序模式排序返回激活订阅。 |
...
`grep -c 'Purpose:' lib/features/finance/views/subscriptions_page.dart` 报告 27，与上面计数的全部 27 个真实声明精确匹配（14 个 Tier A、13 个 Tier B）。每个 `/// Purpose:` 块都恰好位于其文档化的真实声明正上方——未发现错附块（记录调用点而非声明的块），也不存在未文档化的真实声明。与 `analysis_page.dart`（与本文件并列文档化，有一个未文档化方法）不同，本文件的文档注释与其声明完全同步。八个声明在 v1.4.3 离开了本文件：`_sortList`、`_monthlyDue`、`_monthlyAvg`、`_yearlyAvg` 和 `_getUpcomingSubs` 成为 `subscription_summary.dart` 的公开函数，`_buildLeading` 连同它的两个嵌套辅助成为 `SubscriptionAvatar`。
...
- **副作用：** 无——`sortSubscriptions` 返回新列表，因此 `_subscriptions` 永远不会被重排。
- **算法：** `sortSubscriptions(_subscriptions.where((s) => s.isActive).toList(), mode: _sortMode, customOrder: _customOrder)`——[`subscription_summary.md#sortsubscriptions`](../services/subscription_summary.md#sortsubscriptions)。
- **用法：** `final active = _active;`（`build`）。
- **备注：** 自 v1.4.3 起，财务主页的订阅概览用同一个函数和同一个存储的模式排序，因此两份列表的顺序始终一致。每次读取都从头重新过滤和排序；没有缓存。
...
- **算法：**
  ...
  6. 渲染 `ListTile`，`SubscriptionAvatar` 作前导头像、名称/占位标题、金额作尾部文本（主题错误色样式）、`onTap`/`onLongPress` 接到传入回调 / `_showActions`。
- **用法：**
  ```dart
  _SubscriptionTile(
    subscription: sub,
...
- [`subscription_summary.dart`](../services/subscription_summary.md) — `summarizeSubscriptions`、`sortSubscriptions` 和 `upcomingSubscriptions`，自 v1.4.3 起本页与财务主页共享的统计、排序和续费过滤器。
- [`subscription_avatar.dart`](../widgets/subscription_avatar.md) — `SubscriptionAvatar`，图块的前导组件。
- [`currencySymbol`](../services/balance_util.md) — 摘要卡片和图块上的货币符号。
