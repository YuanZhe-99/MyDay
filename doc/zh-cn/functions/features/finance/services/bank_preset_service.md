...
经 `rootBundle.loadString` 加载并缓存 `assets/banks.json` 中的 250+ 捆绑银行/金融科技预设，并提供国家分组、名称搜索、按国家默认币种查找，以及按优先级排序的 logo URL 来源列表供尝试（`BankPreset.logoUrls`），因为没有单个免费 logo API 能可靠覆盖每家银行。自 v1.4.5 起，预设还公开其带国家限定的 `key`，并在完整版构建中公开一个内置标志资源（`bundledLogoAsset`，在生成的 [标志清单](bank_logo_manifest.g.md) 中查找），调用方会先于网络链尝试它。`BankPresetService` 是惰性实例化的单例（`BankPresetService.instance`），因此 JSON 资源每个应用会话最多解析一次。银行预设选择器如何使用它见 [财务](../../../../features/finance.md#bankpresetservice)。
...
| `key` | getter（`BankPreset`） | B | 标志清单使用的唯一预设键 `<country>/<id>`。 |
| [`bundledLogoAsset`](#bundledlogoasset) | getter（`BankPreset`） | A | 此预设的内置标志资源，或 null（商店版构建 / 无标志）。 |
...
**对账：** `grep -c 'Purpose:' lib/features/finance/services/bank_preset_service.dart` 返回 11，与上面 11 个文档化行精确匹配——每个块都恰好位于其真实声明（构造函数、工厂构造函数、getter 或方法）正上方；未发现错附在调用点语句上方。表格在那 11 行之外还有一行：普通 `static const countryCurrency = <String, String>{...}` 字段，不带 `/// Purpose:` 块，与本代码库记录可调用成员而非普通数据字段的约定一致——`shared/services/webdav_service.md` 的未文档化 `static const`/`static final` 字段也见同样模式。对照此列表交叉核对文件中的每个 `class`、`factory`、`get` 和方法声明，没有发现未文档化的可调用声明。`getAll`、`groupedByCountry`、`search` 和两个构造函数分类为 Tier A（真实 IO/缓存或循环逻辑）；`key`、`logoUrl`、`defaultCurrency` 和 `BankPresetService._()` 分类为 Tier B，作为平凡单行访问器/转发构造函数；`logoUrls` 和 `bundledLogoAsset` 尽管是 getter 仍分类为 Tier A，因为它们是 [财务](../../../../features/finance.md#bankpresetservice) 明确点出的标志查找的两层。
...
- **来源：** `lib/features/finance/services/bank_preset_service.dart`（第 22 行）
...
- **来源：** `lib/features/finance/services/bank_preset_service.dart`（第 36 行）
...
- **用法：**
  ...
  ```
  （`lib/features/finance/services/bank_preset_service.dart:133-134`，[`getAll`](#getall) 内。）
...
- **来源：** `lib/features/finance/services/bank_preset_service.dart`（第 58 行）
- **用途：** 返回本构建为此预设附带的内置标志资源（如有）。
- **输入：** 无（读取 `key` 和编译期标志 `bundledBankLogosEnabled`）。
- **返回：** `String?` — 来自 [`bankLogoAssets`](bank_logo_manifest.g.md#banklogoassets) 的 `assets/bank_logos/<country>_<id>.<svg|png>`；在商店版构建中或预设没有审核过的标志时为 null。
- **副作用：** 无。
- **算法：** `bundledBankLogosEnabled ? bankLogoAssets[key] : null`，其中 `key` 是 `'$country/$id'`——带国家限定，因为预设 id 会在不同国家重复（`icbc`、`hsbc`、`vtb`……）。
- **用法：**
  ```dart
  ...
  ```
  （`lib/features/finance/views/accounts_page.dart:2043-2048`，`_fetchBankIcon`；预设选择器中的 [`BankLogoImage`](../widgets/bank_logo_image.md) 也读取它。）
- **备注：** 非 null 值只是清单的承诺，不保证资源能加载——两个读取方都把加载失败当作「没有内置标志」，并回退到 [`logoUrls`](#logourls)。标志如何得出见 [`build_flavor.dart`](../../../app/build_flavor.md)。

### `List<String> get logoUrls` <a id="logourls"></a>
- **种类：** `BankPreset` 的 getter
- **来源：** `lib/features/finance/services/bank_preset_service.dart`（第 67 行）
...
- **用法：**
  ...
  ```
  （`lib/features/finance/views/accounts_page.dart:2049-2054`，`_fetchBankIcon`，按顺序尝试每个 URL 直到下载成功——只有在没有内置标志或复制失败时才会走到这里。）
...
- **来源：** `lib/features/finance/services/bank_preset_service.dart`（第 130 行）
...
- **种类：** `BankPresetService` 的方法
- **来源：** `lib/features/finance/services/bank_preset_service.dart`（第 144 行）
...
- **用法：**
  ...
  ```
  （`lib/features/finance/widgets/bank_preset_picker.dart:107-108`，`_load`，银行选择器的初始分组视图。）
- **备注：** 无。

### `Future<List<BankPreset>> search(String query)` <a id="search"></a>
- **种类：** `BankPresetService` 的方法
- **来源：** `lib/features/finance/services/bank_preset_service.dart`（第 162 行）
...
- **用法：**
  ...
  ```
  （`lib/features/finance/widgets/bank_preset_picker.dart:121-123`，银行选择器的实时搜索处理器。）
