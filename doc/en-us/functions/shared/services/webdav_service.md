...
(`lib/src/webdav/sync_engine.dart` and friends). This file went from 2291 lines to ~240 and kept
every public name and signature, so call sites, the conflict dialogs, and the existing tests are
unchanged.

The five data files — and MyDay's three special cases — are described once in
...
- **Fields:** `todoMerge`, `financeMerge`, `intimacyMerge`, `weightMerge` (the app-typed merge
  results), plus `enginePending` (opaque engine state used by `finalizePendingSync`).
- **Getter:** `allConflicts` — flattens all twelve conflict containers across the four modules.
- **Notes:** Exchange rates merge whole-file and can never conflict, so they never appear here. The
  engine carries each typed merge result through as opaque `state`, which is why the conflict dialogs
  still receive real model objects.

### `progress` <a id="progress"></a>
- **Kind:** static getter → `ValueNotifier<SyncProgress>`.
- **Notes:** Structured modules report an indeterminate upload phase; exchange rates report indexed
  progress. That asymmetry is preserved by a per-module flag in the registry.
...
- **Notes:** A missing or null `remotePath` still defaults to `/MyDay`.
...
- **Side effects:** Acquires the remote `.lock`, then per module in registry order downloads, merges,
  applies the module's transforms, uploads, and saves the base; then syncs referenced finance and
  intimacy images; updates `progress`.
...
- **Notes:** Each module's `buildResolved` picks out the records it recognizes by runtime type. The
  finance forced-balance migration still runs after resolution, and unknown-field preservation is
  still applied immediately before the write.
