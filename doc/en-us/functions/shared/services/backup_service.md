...
(`lib/src/backup/backup_engine.dart`). This file kept every public name and signature, so the
...
**MyDay-specific:** the daily auto-backup is driven by `ReminderService`'s 30-second loop, **not** by
the auto-sync scheduler. See [`auto_sync_service.md`](auto_sync_service.md).
...
- **Purpose:** Maps data-file name to backup module key for the five modules.
- **Notes:** Derived from the module registry — one of the five hardcoded copies of the file list
  that the extraction removed.
...
- **Notes:** No-op when `autoBackupEnabled` is false. Re-entrancy guarded. "Already backed up today"
  ...
  Called from `ReminderService`'s 30-second loop.
...
- **Returns:** Module ids in registry order; empty for an unparseable bundle.
